...
#This script will monitoring the CPU, RAM, Storage, Process and Port usage of the system
#Variables 
CPU_THRESHOLD=80
RAM_THRESHOLD=80
...
EMAIL_ID="Changetheemailid"
APP_PASSWORD="ChangeThisToYourAppPassword"

echo "CPU, RAM, Storage, Process and Port monitoring of the system"
...
for process in $CRITICAL_PROCESSES; do
    if ! pgrep "$process" > /dev/null; then
        send_email "Process Alert" "Critical process '$process' is not running"
    fi
done
...
for port in $CRITICAL_PORTS; do
    if ! netstat -tuln | grep ":$port " > /dev/null; then
        send_email "Port Alert" "Critical port $port is not listening"
    fi
done
